...
**Status atual:** vigesimo quarto corte entregue em navegacao, foco, preferencias visuais do sistema e copy acessivel para screen reader. Alem de `skip links`, landmarks/focus targets, `focus-visible`, `prefers-reduced-motion`, `prefers-contrast`, thread announcements, `aria-live` nos toasts, regiao acessivel no reader, `focus management` nos dialogs e navegacao por setas na thread list, o reader agora tambem permite navegar entre mensagens com `ArrowUp`, `ArrowDown`, `Home` e `End`, o shell recebeu um reforco nos tokens de contraste/legibilidade para melhorar leitura de bordas, superficies e texto secundario, a base de tipografia passou a respeitar explicitamente `text-size-adjust` do sistema/navegador com microcopy minima um pouco menos comprimida, a busca estruturada do topbar agora funciona como `combobox` navegavel por teclado com `ArrowUp`, `ArrowDown`, `Enter` e `Escape`, o divisor entre thread list e reader no layout split tambem passou a responder a teclado com `ArrowLeft`, `ArrowRight`, `Home` e `End`, os dialogs auxiliares de templates no composer agora entram no mesmo fluxo de foco inicial e fechamento com `Escape`, o menu de contexto da thread list tambem pode ser aberto por teclado com `Shift+F10` / tecla Menu e navegado com setas, `Home`, `End` e `Escape`, os `contact cards` do reader/composer agora expõem `aria-haspopup`/`aria-expanded` no trigger e permitem fechar com `Escape` sem depender de hover, os controles primarios de onboarding, busca, composer e reader passaram a expor nomes acessiveis mais explicitos para leitores de tela e testes de teclado, a sidebar agora tambem permite navegar entre pastas e labels com `ArrowUp`, `ArrowDown`, `Home` e `End`, a navegacao lateral de `Preferences` entrou no mesmo padrao com setas e saltos de inicio/fim entre secoes, a lista de contatos em `Preferences` agora tambem aceita navegacao por setas com `Home`/`End`, a lista de templates passou a seguir o mesmo padrao de `listbox` com navegacao por teclado, a lista de regras ganhou o mesmo comportamento de foco estruturado, a lista de contas tambem entrou nesse fluxo com setas e saltos de inicio/fim, a lista de plugins agora acompanha o mesmo padrao, o seletor de tema virou um `radiogroup` navegavel por setas, e os filtros rapidos da inbox agora tambem seguem semantica de `radiogroup` com roving focus via teclado. Isso ainda nao fecha a fase de a11y, mas deixa o fluxo keyboard-only e a leitura visual mais consistentes entre inbox, settings e leitura.
